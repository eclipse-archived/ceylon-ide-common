...
test shared void testImports() {
    ...
    // import keyword
    assertContains(result, Result("newKeywordCompletionProposal", "import"));
...
test shared void testAnonymousFunctions() {
    ...
    assertEquals(result.first, Result("newAnonFunctionProposal", "(Character a) => nothing"));
    assertEquals(result.get(1), Result("newAnonFunctionProposal", "void (Character a) {}"));
}
