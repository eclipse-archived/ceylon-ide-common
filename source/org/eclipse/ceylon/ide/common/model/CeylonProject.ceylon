...
import org.eclipse.ceylon.tools.bootstrap {
    CeylonBootstrapTool
}
...
shared abstract class BaseCeylonProject() {
    ...
    "
     ...
     [[IdeUtils.newOperationCanceledException|org.eclipse.ceylon.ide.common.platform::IdeUtils.newOperationCanceledException]]
     "
        ...
        do() => withCeylonModelCaching(() {
            try (progress = mon.Progress(113, "Setting up typechecker for project ``name``")) {
                ...
                if (exists jdkProvider = configuration.jdkProvider) {
                    let ([name, *rest] = jdkProvider.split('/'.equals).sequence());
