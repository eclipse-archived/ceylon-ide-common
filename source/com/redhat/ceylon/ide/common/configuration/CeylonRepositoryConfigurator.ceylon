...
shared abstract class CeylonRepositoryConfigurator() {
	...
	shared void addAetherRepo(String repo) {
		Integer index = projectLocalRepos.size + globalLookupRepos.size + projectRemoteRepos.size;
		if (repo.empty) { addProjectRepo("aether", index, false); } else { addProjectRepo("aether:" + repo, index, false); }
	}
	...
	shared Boolean isRepoConfigurationModified(CeylonProjectConfig<out Object> config) { return !(projectLocalRepos.equals(config.projectLocalRepos) && projectRemoteRepos.equals(config.projectRemoteRepos)); }
	shared void removeSelectedRepo() {
		...
		for (Integer i in 0:selection.size) {
			...
			if (exists index) { if (!isFixedRepoIndex(index)) {
	...
	void updateRemoveRepoButtonState() {
		...
		for (value index in selectionIndices) { if (!isFixedRepoIndex(index)) {
	...
	void updateUpDownButtonState() {
		...
		if (selectionIndices.size == 1) {
			...
			if (exists index) {
				if (index>0 && !isFixedRepoIndex(index)) { isUpEnabled = true; }
				value maxIndex = projectLocalRepos.size + globalLookupRepos.size + projectRemoteRepos.size - 1;
				if (index<maxIndex && !isFixedRepoIndex(index)) { isDownEnabled = true; }
			}
		}
	...
	void addProjectRepo(String repo, Integer index, Boolean isLocalRepo) {
		if (isLocalRepo && projectLocalRepos.contains(repo)) { return; }
		if (!isLocalRepo && projectRemoteRepos.contains(repo)) { return; }
		if (isLocalRepo) { projectLocalRepos.insert(index, repo); } else {
