...
List<[Pattern,String]> singularizations = ArrayList {
    ...
    [pattern("(.*)ices$"), "$1ex"],
    [pattern("(.*(vir|fung|syllab|nucle|stimul|foc|termin|radi|styl))i$"), "$1us"],
    [pattern("(.*(phenomen|criteri|automat|polyhedr))a$"), "$1on"],
    [pattern("(.*(bacteri|curricul|medi|memorand|addend|for|millenni|referend|spectr))a$"), "$1um"],
    //[pattern("(.+)ses"), "$1sis"],
...
List<[Pattern,String]> pluralizations = ArrayList {
    ...
    [pattern("(.+)(e|i)x$"), "$1ices"],
    [pattern("(.*(vir|fung|syllab|nucle|stimul|foc|termin|radi|styl))us$"), "$1i"],
    [pattern("(.*(phenomen|criteri|automat|polyhedr))on$"), "$1a"],
    [pattern("(.*(bacteri|curricul|medi|memorand|addend|for|millenni|referend|spectr))um$"), "$1a"],
    [pattern("(.+)sis"), "$1ses"],
